...
Phase 5ループの最終ステップ。Step 9（仮説生成）とStep 10（検証）の結果を統合し、
insights.md / golden_theory.json を更新。収束判定を行う。

実行方法:
  python scripts/step11_integrate.py --integrate
"""
...
def run_data_summarizer(diff_video_id=None):
    """step3_summarize.py を実行"""
    cmd = [sys.executable, os.path.join(os.path.dirname(__file__), "step3_summarize.py")]
    if diff_video_id:
...
def run_step3():
    """step8_build_model.py を実行"""
    cmd = [sys.executable, os.path.join(os.path.dirname(__file__), "step8_build_model.py")]
    print("\n[統合] モデル再構築中...")
    result = subprocess.run(cmd, capture_output=True, text=True)
    print(result.stdout)
    if result.returncode != 0:
...
def generate_conclusion_report():
    ...
    lines.append("\n---")
    lines.append("\n> このレポートは `python scripts/step11_integrate.py --integrate` の実行時に自動生成されます。")
...
def generate_production_feedback():
    ...
    lines.append("\n---")
    lines.append(f"\n> 自動生成: `python scripts/step11_integrate.py --integrate` (W-22)")
