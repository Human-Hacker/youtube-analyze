...
| W-24 | `youtube-feedback/`（新規リポジトリ）+ Agent F/G | **新規作成** | youtube-analyze → youtube-long 変更戦略を策定する独立リポジトリ。Agent F（分析）+ Agent G（検証）。人間起動 |
...
### W-24. Agent F（フィードバック分析）+ Agent G（フィードバック検証）— 独立ワークフロー

#### 24-a. 背景と設計判断

youtube-analyzeの分析結果をyoutube-longの制作パイプラインに反映するには、2つの異なるレベルの作業が必要:

| レベル | 内容 | 担当 |
|--------|------|------|
| **数値差分検出** | model.jsonの閾値・回帰式とselection_report.mdの記載値の差異を機械的に検出 | W-22 `generate_production_feedback()`（Python） |
| **戦略的変更分析** | 「この分析発見はyoutube-longのどのファイルのどの部分にどう影響し、どう変更すべきか」を推論 | **W-24 Agent F/G**（LLMエージェント） |

**W-22との関係**: W-22は廃止せず、Agent Fへの**入力前処理**として残す。W-22が生成する `production_feedback.md` の数値差分テーブルは、Agent Fが参照する入力の一つになる。

**設計判断（4項目）**:

1. **W-22との関係**: 役割が異なるため共存。W-22=機械的差分、Agent F/G=戦略的分析
2. **配置**: youtube-analyzeにもyoutube-longにも属さない**独立した仕組み**
3. **起動タイミング**: **人間が明示的に起動**。自動実行はしない
4. **Agent C/Eとの関係**: **独立した別のワークフロー**。Agent C/Eは分析サイクル内で完結、Agent F/Gは分析サイクル完了後に別プロセスとして実行

#### 24-b. ワークフロー全体像

```
youtube-analyze 分析サイクル完了
    │
    ├─ 成果物: golden_theory.json, model.json, insights.md, analysis_conclusion.md
    ├─ W-22自動生成: workspace/production_feedback.md（数値差分）
    │
    ▼
【人間が判断】分析結果をyoutube-longに反映する必要があるか？
    │
    │  YES
    ▼
┌─────────────────────────────────────────────────┐
│ Phase 1: 変更戦略の策定（Agent F）               │
│                                                   │
│  入力:                                            │
│    [youtube-analyze側]                            │
│    - data/golden_theory.json                      │
│    - data/model.json                              │
│    - data/analysis_history/insights.md            │
│    - data/workspace/analysis_conclusion.md        │
│    - data/workspace/production_feedback.md (W-22) │
│    - data/analysis_fundamentals.json              │
│                                                   │
│    [youtube-long側]                               │
│    - prompts/ 全ファイル（10ステップ）             │
│    - prompts/*_regulation.md（5レギュレーション）  │
│    - prompts/regulation_id_registry.md            │
│    - prompts/feedback_regulation_mapping.md       │
│    - prompts/system_prompt.md                     │
│                                                   │
│  処理:                                            │
│    Step 1. 分析結果の要約（何が分かったか）        │
│    Step 2. 影響マッピング（どこに影響するか）      │
│    Step 3. 変更案の策定（どう変えるべきか）        │
│    Step 4. 優先度・リスク評価                      │
│                                                   │
│  出力: change_strategy.md                         │
└────────────────────┬────────────────────────────┘
                     │
                     ▼
┌─────────────────────────────────────────────────┐
│ Phase 2: 変更戦略の検証（Agent G）               │
│                                                   │
│  入力:                                            │
│    - Agent F の change_strategy.md                │
│    - Agent F が参照した全入力ファイル（同一セット）│
│                                                   │
│  検証項目:                                        │
│    V1. 根拠の正当性（分析データが変更を裏付けるか）│
│    V2. 影響の網羅性（見落としている影響はないか）  │
│    V3. 矛盾の検出（既存regulationとの衝突はないか）│
│    V4. 優先度の妥当性                              │
│    V5. 4層ルール階層の整合性                       │
│                                                   │
│  出力: feedback_verification_report.md            │
│    各変更案を approved / modified / rejected で判定│
└────────────────────┬────────────────────────────┘
                     │
                     ▼
【人間がレビュー】feedback_verification_report.md を確認
    │
    ├─ 修正が必要 → Agent F を再実行（Agent G の指摘を反映）
    │
    └─ 承認 → Phase 3 へ
                     │
                     ▼
┌─────────────────────────────────────────────────┐
│ Phase 3: 変更の適用（人間 or 既存フィードバック機構）│
│                                                   │
│  方法1: feedback_regulation_mapping.md のフローで  │
│         既存のフィードバック恒久化エージェントを使用│
│  方法2: 人間が手動で該当ファイルを編集             │
│                                                   │
│  ※ Agent F/G は変更戦略の策定・検証まで。          │
│    実際のファイル編集はスコープ外。                 │
└─────────────────────────────────────────────────┘
```

#### 24-c. Agent F: 変更戦略分析エージェント

**役割**: youtube-analyzeの分析結果を読み込み、youtube-longパイプラインの**どのファイルの・どの部分を・なぜ・どのように**変更すべきかを戦略的に分析する。

**出力ファイル**: `youtube-analyze/data/workspace/change_strategy.md`
...
> 生成日時: YYYY-MM-DD
> 基盤分析サイクル: N回目
> 前提: analysis_conclusion.md のサマリー
...
### 0-1. 今サイクルの主要発見
（analysis_conclusion.md + golden_theory.json から要約）

### 0-2. W-22数値差分（production_feedback.md から転記）
（GI×CA閾値、回帰式、相関値の差異テーブル）

---

## 1. 影響マッピング

（分析発見ごとに、影響を受けるyoutube-longファイルをマッピング）

### 発見1: {発見の要約}

| 影響先ファイル | 影響箇所 | 影響の種類 | 重大度 |
|---------------|---------|-----------|--------|
| prompts/1. selection_report.md | GI×CA判定基準 | 数値更新 | 高 |
| prompts/system_prompt.md | SP-0XX | ルール追加 | 中 |
| ... | ... | ... | ... |
...
- **対象ファイル**: prompts/1. selection_report.md
- **対象箇所**: 「GI×CA判定基準」セクション
- **現在の記載**:
  ```
  （現在のファイルから該当箇所を引用）
  ```
- **提案する変更**:
  ...
  （変更後の記載案）
  ```
- **変更の根拠**: golden_theory.jsonのP1原則 + model.jsonの閾値精度XX%
- **分析データの裏付け**: {具体的なデータ参照}
- **リスク**: {変更による副作用の可能性}
- **優先度**: 高 / 中 / 低
- **適用方法**: 手動編集 / feedback_regulation_mapping.mdフロー
...
（分析した結果、変更不要と判断したファイルとその理由）

| ファイル | 変更不要の理由 |
|---------|---------------|
| prompts/6. event2event_research.md | 今サイクルの発見はリサーチ深度に関連しない |
| ... | ... |
...
| 順位 | 変更案ID | 対象 | 理由 |
|------|---------|------|------|
| 1 | CS-001 | selection_report.md | 数値乖離が大きく即座の反映が必要 |
| 2 | CS-003 | common_regulation.md | 新チェックリスト条件の恒久化 |
| ... | ... | ... | ... |

---

## 5. youtube-long 4層ルール階層との整合性

（変更案が4層構造のどこに位置するかを明示）

| 変更案ID | ルール階層 | 理由 |
|---------|-----------|------|
| CS-001 | inline_rule（プロンプト内） | 数値の直接更新のみ |
| CS-003 | common_regulation | 全パートに適用されるルール |
| CS-005 | system_prompt | 台本の根本方針に関わる |
```
...
| Step | 名前 | 入力 | 処理 | 出力 |
|------|------|------|------|------|
| 1 | 分析結果の構造化理解 | golden_theory.json, insights.md, analysis_conclusion.md, production_feedback.md | 今サイクルで何が分かったかを整理。新規発見・更新された仮説・棄却された仮説を分類 | セクション0（分析結果サマリー） |
| 2 | 影響マッピング | Step 1の要約 + youtube-long全プロンプト・レギュレーション | 各発見に対して、影響を受けるyoutube-longファイルと箇所を特定。影響の種類（数値更新 / ルール追加 / ルール修正 / ルール削除 / ガイドライン変更）を分類 | セクション1（影響マッピング） |
| 3 | 変更案の策定 | Step 2のマッピング + 該当ファイルの現在の内容 | 各影響箇所に対して具体的な変更案を策定。現在の記載と提案する変更を対比で記載。根拠・リスク・優先度を付記 | セクション2-4（変更案一覧・除外リスト・優先順位） |
| 4 | 4層ルール階層の整合性確認 | Step 3の変更案 + system_prompt.md, *_regulation.md | 各変更案が4層構造（system_prompt > common_regulation > part_regulation > inline_rule）のどこに位置するか確認。上位ルールとの矛盾がないか検証 | セクション5（階層整合性） |

**Agent Fの制約事項**:

1. **ファイル編集禁止**: Agent Fはyoutube-longのファイルを一切編集しない。変更戦略書の出力のみ
2. **仮説駆動**: analysis_fundamentals.jsonの方法論に従い、「なぜこの変更が必要か」の仮説を立て、分析データで裏付ける
3. **棄却仮説の活用**: insights.mdの棄却仮説を確認し、「やっても意味がないこと」をyoutube-long側でも排除する提案を含める
4. **regulation_id_registry.mdの尊重**: 新ルール追加時は既存IDと衝突しないことを確認

#### 24-d. Agent G: 変更戦略検証エージェント

**役割**: Agent Fが策定した変更戦略書の正当性・網羅性・整合性を検証し、判定結果をAgent Fに返す（レッドチーム的役割）。

**出力ファイル**: `youtube-analyze/data/workspace/feedback_verification_report.md`

**検証の5軸**:

| 軸 | 検証内容 | 判定 |
|----|---------|------|
| V1. 根拠の正当性 | 各変更案の「根拠」が分析データ（golden_theory.json, model.json, insights.md）で実際に裏付けられているか。Agent Fが分析データを誤読・拡大解釈していないか | ✅ approved / ⚠️ weak_evidence / ❌ unsupported |
| V2. 影響の網羅性 | Agent Fが見落としている影響先はないか。youtube-longの全10ステップ + 全5レギュレーションを独立に走査し、Agent Fのマッピングと照合 | ✅ complete / ⚠️ missing_impacts: [リスト] |
| V3. 既存ルールとの矛盾 | 提案された変更が、既存のsystem_prompt / common_regulation / part_regulationと矛盾しないか。変更Aと変更Bが互いに矛盾しないか | ✅ consistent / ❌ conflicts: [リスト] |
| V4. 優先度の妥当性 | Agent Fが設定した優先度が、分析データの重要度・影響範囲と整合しているか | ✅ appropriate / ⚠️ suggested_reorder: [リスト] |
| V5. 4層階層の整合性 | 変更案が適切なルール階層に配置されているか。上位ルールを下位で矛盾なく実装できるか | ✅ correct / ❌ hierarchy_violations: [リスト] |
...
# 変更戦略 検証レポート
> 検証日時: YYYY-MM-DD
> 検証対象: change_strategy.md (YYYY-MM-DD生成)
...
| 検証軸 | 判定 | コメント |
|--------|------|---------|
| V1. 根拠の正当性 | ⚠️ | CS-003の根拠が弱い |
| V2. 影響の網羅性 | ⚠️ | Step 6への影響が見落とし |
| V3. 既存ルールとの矛盾 | ✅ | 矛盾なし |
| V4. 優先度の妥当性 | ✅ | 適切 |
| V5. 4層階層の整合性 | ✅ | 適切 |

**推奨アクション**: Agent F に V1, V2 の指摘事項を反映して再策定を依頼
...
- **V1 根拠**: ✅ approved — model.jsonの閾値精度XX%がselection_report.mdの記載値と乖離。データで裏付けられている
- **V2 網羅性**: ✅ — 影響先は1ファイルのみで正しい
- **V3 矛盾**: ✅ — 既存ルールと矛盾なし
- **V5 階層**: ✅ — inline_ruleの数値更新であり適切

### CS-003: {変更の要約}

- **V1 根拠**: ⚠️ weak_evidence — insights.mdのINS-002-H3を根拠としているが、この仮説はstatus=hypothesisであり未検証。確定した知見ではない
- **推奨**: golden_theory.jsonのchecklist status=adoptedの条件のみを根拠とすべき
- **V2 網羅性**: ⚠️ missing_impacts — common_regulation.mdへの追加を提案しているが、intro_regulation.mdのIR-005と競合する可能性。Agent Fの分析ではintro_regulation.mdが検討されていない

（以下、各変更案について同様の構造）

---

## 見落とし指摘（Agent Fの分析に含まれていない影響）

| # | 発見 | 見落とされた影響先 | 説明 |
|---|------|------------------|------|
| M1 | {分析発見X} | prompts/6. event2event_research.md | Agent Fはこのファイルを「変更不要」としたが、{理由}により影響がある可能性 |

---

## Agent F への返却指示

1. CS-003 の根拠を強化するか、優先度を下げること
2. M1 の影響を調査し、必要なら変更案を追加すること
3. 上記を反映した改訂版 change_strategy.md を再策定すること
```

#### 24-e. W-22 との統合

W-22の `generate_production_feedback()` は以下のように位置づけを変更:

| 項目 | W-22（変更前） | W-22（W-24統合後） |
|------|---------------|-------------------|
| 目的 | 分析結果→youtube-longへの更新提案を自動生成 | Agent Fへの**数値差分入力**を自動生成 |
| 出力 | production_feedback.md（3セクション構成の完成提案） | production_feedback.md（**セクション1のみ**: 数値差分テーブル） |
| セクション2-3 | golden_theory→制作アクション変換、レビュー基準提案 | **Agent Fに移管**。LLMの戦略的推論で実施 |
| 実行タイミング | --integrate完了時に自動 | 変更なし（自動生成。Agent F実行前に生成済みであること） |
| 読み手 | 人間 | **Agent F**（+ 人間） |

**理由**: セクション2-3（制作アクション変換・レビュー基準提案）はPythonの機械的変換ルールでは不十分。例えば「感情の底を3回以上設計する」というチェックリスト条件をyoutube-longのどのステップのどのレギュレーションに反映すべきかは、台本設計の文脈理解が必要であり、LLM（Agent F）の方が適している。

W-22の22-aテンプレートは以下のように簡略化:
...
## 数値差分テーブル（selection_report.md vs model.json）

| 項目 | selection_report.md記載値 | model.json最新値 | 差異 | 乖離率 |
|------|--------------------------|-----------------|------|--------|
{自動生成}
...
#### 24-f. エージェント定義ファイルの配置

Agent F/Gはyoutube-analyzeにもyoutube-longにも属さない**完全独立のリポジトリ**として、`タスク/` 直下に配置する:

```
タスク/
├── youtube-analyze/                         # 分析パイプライン（入力元）
│   ├── agents/
│   │   ├── analyze-step4-hypothesis.md      # Agent C
│   │   └── analyze-step5-verification.md    # Agent E
│   ├── prompts/
│   │   ├── coordinator.md
│   │   ├── youtube_fundamentals.md
│   │   └── scoring_criteria.md
│   ├── data/
│   │   ├── golden_theory.json
│   │   ├── model.json
│   │   ├── workspace/production_feedback.md # W-22出力（Agent Fの入力）
│   │   ├── output/analysis_conclusion.md
│   │   └── history/insights.md
│   └── ...
│
├── youtube-long/                            # 制作パイプライン（変更先）
│   ├── prompts/                             # Agent Fが走査する対象
│   │   ├── 1. selection_report.md
│   │   ├── ...（10ステップ）
│   │   ├── system_prompt.md
│   │   ├── *_regulation.md
│   │   ├── regulation_id_registry.md
│   │   └── feedback_regulation_mapping.md
│   └── ...
│
├── youtube-feedback/                        # ← 新規。完全独立リポジトリ
│   ├── agents/
│   │   ├── feedback-step1-analyzer.md       # Agent F（変更戦略分析）
│   │   └── feedback-step2-verifier.md       # Agent G（変更戦略検証）
│   ├── prompts/
│   │   └── feedback_coordinator.md          # ワークフロー制御
│   ├── data/
│   │   └── workspace/
│   │       ├── change_strategy.md           # Agent F出力
│   │       └── verification_report.md       # Agent G出力
│   └── commands/
│       └── feedback-pipeline.md             # 起動コマンド
│
├── youtube-automation/
└── youtube-shorts/
```
...
1. **完全独立リポジトリ**: youtube-analyzeの成果物を**読む**、youtube-longの構造を**走査する**が、どちらにも**書き込まない**。出力は自身の `data/workspace/` にのみ書く
2. **命名規則**: 既存の `youtube-*` パターン（youtube-analyze, youtube-long, youtube-shorts, youtube-automation）に合わせて `youtube-feedback` とする
3. **ディレクトリ構造**: youtube-analyzeのパターン（agents/, prompts/, data/, commands/）に準拠
4. **ファイル名衝突なし**: Agent G出力が `verification_report.md` でも、youtube-feedback/data/workspace/ に置くため youtube-analyze 側のAgent E出力と衝突しない

**youtube-feedback が参照するパス（config相当）**:

```
# 入力元: youtube-analyze（読み取り専用）
YOUTUBE_ANALYZE_DIR = ../youtube-analyze/
  - data/golden_theory.json
  - data/model.json
  - data/history/insights.md
  - data/output/analysis_conclusion.md
  - data/workspace/production_feedback.md
  - data/analysis_fundamentals.json

# 走査対象: youtube-long（読み取り専用）
YOUTUBE_LONG_DIR = ../youtube-long/
  - prompts/（動的走査。ハードコーディングしない）
...
**feedback_coordinator.md の役割**:

```markdown
# フィードバックワークフロー コーディネーター

## 目的
youtube-analyzeの分析結果をyoutube-longの制作パイプラインに反映するための
変更戦略の策定と検証を制御する。

## 参照先
- 入力元: ../youtube-analyze/（読み取り専用）
- 走査対象: ../youtube-long/（読み取り専用）
- 出力先: data/workspace/（本リポジトリ内）

## 前提条件（実行前に確認）
- [ ] youtube-analyze の分析サイクルが完了していること
- [ ] youtube-analyze/data/output/analysis_conclusion.md が生成済みであること
- [ ] youtube-analyze/data/workspace/production_feedback.md が生成済みであること（W-22）
- [ ] youtube-analyze/data/golden_theory.json が最新サイクルの結果を反映していること

## ワークフロー

### Phase 1: Agent F（変更戦略の策定）
1. Agent F を起動し、data/workspace/change_strategy.md を生成させる
2. 人間が change_strategy.md をレビュー（任意）

### Phase 2: Agent G（変更戦略の検証）
3. Agent G を起動し、data/workspace/verification_report.md を生成させる
4. 人間が verification_report.md を確認

### Phase 3: 修正ループ（必要な場合）
5. Agent G の指摘が「修正必要」の場合:
   - verification_report.md を Agent F に渡して再策定
   - 再策定後の change_strategy.md を Agent G で再検証
   - 「全変更案が approved」になるまで繰り返す

### Phase 4: 適用（人間が実施）
6. 最終承認された change_strategy.md に基づき:
   - 方法1: youtube-long の feedback_regulation_mapping.md フローで適用
   - 方法2: 人間が手動で該当ファイルを編集
```

#### 24-g. youtube-long 構造変更への耐性設計

**問題**: youtube-longのプロンプト・レギュレーション・エージェントファイルは頻繁に変更される（ステップの追加・統合・分割、レギュレーションID体系の変更、ファイル名のリネーム等）。Agent F/Gの定義ファイルにyoutube-longの構造をハードコーディングすると、変更のたびにAgent F/Gの定義も更新が必要になり、陳腐化リスクが高い。

**設計原則: Agent F/Gはyoutube-longの構造を「知識」として持たず、「実行時に走査」する**

具体的には:

**1. 動的ファイル走査（ハードコーディング禁止）**

Agent Fは実行時に以下の走査を行い、youtube-longの**現在の構造**を把握する:

```markdown
## Agent F 実行時 Step 0: youtube-long 構造の動的把握

### 0-1. ファイル走査
以下のディレクトリを走査し、現在存在するファイルの一覧を取得する:
- youtube-long/prompts/*.md          → 全プロンプトファイル
- youtube-long/prompts/*_regulation.md → 全レギュレーションファイル
- youtube-long/agents/*.md           → 全エージェントファイル

### 0-2. 構造マップの自動生成
走査結果から以下を自動的に把握する:
- パイプラインのステップ数と順序（ファイル名の番号プレフィックスから推定）
- レギュレーションファイルの一覧とID体系（各ファイルの冒頭を読んでID接頭辞を確認）
- ルール階層の構造（system_prompt → common → part-specific の関係を確認）

### 0-3. 参照すべきメタファイルの確認
以下のメタファイルが存在すれば読み込む（存在しなくても動作する）:
- youtube-long/prompts/regulation_id_registry.md → ID体系の把握
- youtube-long/prompts/feedback_regulation_mapping.md → フィードバック適用経路の把握
- youtube-long/README*.md → パイプライン全体の概要把握
```

**2. Agent定義ファイルに書く内容 vs 書かない内容**

| 書く内容（不変の原則） | 書かない内容（変動する構造） |
|----------------------|--------------------------|
| 分析発見をパイプラインにマッピングする**方法論** | 具体的なステップ名やファイル名 |
| 4層ルール階層の**概念**（上位 > 下位の原則） | 各階層に属する具体的なルールID |
| 変更案を策定する際の**評価軸** | youtube-longの特定プロンプトの内容 |
| Agent Gの検証5軸（V1-V5）の**基準** | 特定レギュレーションの条件文 |
| 走査手順（Step 0） | 走査結果のキャッシュ |

**3. 4層ルール階層の理解方法**

Agent Fは以下の**発見ルール**でルール階層を動的に判定する:

```markdown
## ルール階層の動的判定

1. system_prompt.md が存在するか確認 → 最上位ルール
2. common_regulation.md が存在するか確認 → 全パート共通ルール
3. *_regulation.md（common以外）を列挙 → パート別ルール
4. 各ステッププロンプト内の記述 → インラインルール
5. ファイル間の参照関係（「system_promptに従う」等の記述）から上下関係を確認

※ ファイル名が変わっても、内容の読み取りで階層関係を判定できる
```

**4. 変更検知と警告**

Agent Fの出力（change_strategy.md）のセクション0に「構造スナップショット」を含める:

```markdown
## 0-0. youtube-long 構造スナップショット（本分析時点）
...
> ⚠ 前回のchange_strategy.md と構造が異なる場合、
> 変更された箇所を明示し、影響マッピングの再評価が必要か判断すること
```

Agent Gの検証項目V2（影響の網羅性）でも、この構造スナップショットを使い、Agent Fが**現在存在する全ファイル**を走査対象にしたかを検証する。

**5. 参考情報としての現時点の構造（2025年時点）**

> 以下は本設計書作成時点のyoutube-long構造であり、**Agent F/Gの定義ファイルには記載しない**。
> Agent F/G開発者の理解用としてのみ記載する。

現時点の10ステップパイプライン:

| Step | ファイル | 概要 |
|------|---------|------|
| 1 | 1. selection_report.md | GI×CA選定 + 好奇心マッピング |
| 2 | 2. research2themes.md | リサーチ→テーマ変換 |
| 3 | 3. theme2character.md | テーマ→キャラクター設計 |
| 4 | 4. character2hook.md | キャラクター→フック設計 |
| 5 | 5. hook2event.md | フック→イベント抽出 |
| 6 | 6. event2event_research.md | 密度の高いリサーチ |
| 7a | 7a. event_placement.md | ビート配置 |
| 7b | 7b. event_placement2three_act_structure.md | 三幕構成 |
| 9 | 9_review_common.md + 9a-9d | レビュー |
| 10 | 10_rewrite_common.md + 10a-10d | リライト |

現時点の4層ルール階層:

```
system_prompt.md          ← 最上位
  ├── common_regulation.md  ← 全パート共通
  │     ├── intro_regulation.md    ← 導入パート専用
  │     ├── first_regulation.md    ← 第一幕専用
  │     ├── second_regulation.md   ← 第二幕専用
  │     └── third_regulation.md    ← 第三幕専用
  └── 各ステッププロンプト内のインラインルール  ← 最下位
```

ルール追加時の原則: 上位ルールは下位で矛盾なく実装できること。下位ルールは上位ルールの範囲内であること。

#### 24-h. 問題対応マッピングへの追加

| # | 元の問題 | 対応する変更 | 解決方法 |
|---|---------|------------|---------|
| 24 | 分析結果→youtube-long変更が「どう変えるべきか」の戦略的判断なしに行われる | W-24 | Agent F が分析結果を戦略的に解釈し変更案を策定。Agent G がレッドチームとして検証。W-22は数値差分の前処理に縮小 |
...
--- W-24: Agent F/G フィードバックワークフロー（youtube-feedback/ リポジトリ新設）---
48. タスク/youtube-feedback/ ディレクトリを新規作成（agents/, prompts/, data/workspace/, commands/）
49. youtube-feedback/prompts/feedback_coordinator.md を新規作成（W-24）
50. youtube-feedback/agents/feedback-step1-analyzer.md を新規作成（W-24: Agent F）
51. youtube-feedback/agents/feedback-step2-verifier.md を新規作成（W-24: Agent G）
52. youtube-feedback/commands/feedback-pipeline.md を新規作成（W-24: 起動コマンド）
53. W-22の22-aテンプレートをセクション1（数値差分）のみに簡略化（W-24）

--- 統合テスト ---
54. python scripts/step3_build_model.py        ← 分割後の動作確認（fundamentals整合性チェック含む）
55. python scripts/step2_analyze.py             ← データ準備の動作確認（fundamentals整合性チェック含む）
56. Agent C/E を1サイクル手動実行               ← 出力フォーマット + fundamentals参照の確認
57. python scripts/step2_analyze.py --integrate ← 統合処理 + 方法論レビュー + 結論レポート + production_feedback生成の確認
58. python scripts/step4_pdca.py TEST_ID --skip-fetch ← PDCA出力先の確認
59. youtube-feedback/ で Agent F/G を手動実行   ← change_strategy.md + verification_report.md の生成確認（W-24）
```
...
| 24 | 分析結果→youtube-long変更が戦略的判断なしに行われる（W-22は数値差分のみ） | W-24 | Agent Fが分析結果を戦略的に解釈し変更戦略書を策定。Agent Gがレッドチームとして検証。W-22は数値差分の前処理に縮小 |
